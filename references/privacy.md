...
- Match a blur to the selected element's measured bounds. Use a strong neutral Gaussian blur on the element itself, without a branded tint, outline, shadow, or larger decorative plate. Preserve the element's own corner shape. Clip the effect to the target so it never covers a non-sensitive label, icon, button, adjacent field, or other sibling. If a sibling is also sensitive, give it its own measured blur; use one parent blur only when every visible child inside that parent needs protection. This mirrors Loom's element-bound treatment and keeps the protection from looking like a separate UI control.
...
- Start from the exact measured element bounds. Blur defaults to zero padding; do not enlarge the region to create softness. If antialiasing requires a small margin, cap it at two pixels and reduce it to zero before it can overlap a sibling element. Sample extra source pixels for the filter, then clip the rendered blur to the protected element. Solid covers may keep wider safety padding.
...
- For ordinary personal data, use a strong neutral Gaussian blur that preserves the layout without exposing the value. Do not add a tint, border, shadow, or replacement field unless the source element already has it. Verify that the result stays unreadable at full resolution.
