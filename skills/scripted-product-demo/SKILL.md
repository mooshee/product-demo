...
Use fictional or synthetic records whenever possible. Choose privacy handling from the source: for a new capture, hide sensitive pixels before the first frame is recorded; for existing footage, apply tracked source-coordinate blur in post-production. Keep secrets behind opaque replacements even when ordinary personal data uses blur. Light blur and coarse pixelation can leak shape, length, or content. Read [references/privacy.md](references/privacy.md) before recording or editing any authenticated product, customer-like record, notification surface, or account screen.
